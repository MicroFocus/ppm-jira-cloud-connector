...
package com.ppm.integration.agilesdk.connector.jira.cloud.service;

import com.hp.ppm.user.model.User;
import com.ppm.integration.agilesdk.ValueSet;
import com.ppm.integration.agilesdk.connector.jira.cloud.model.*;
...
public class JIRAService {
    ...
    public List<JIRAIssueType> getProjectIssueTypes(String projectKey) {
        ...
        if ("*".equals(projectKey)) {
            ...
            // We must only return global issue types - if they have scope (i.e. are likely project specific), it means they are an issue type of a team-managed Project..
    ...
    private void setJiraJSonFields(JSONObject fieldsObj, Map<String, String> fields, String projectKey,
        ...
        for (Map.Entry<String, String> fieldEntry : fields.entrySet()) {
            ...
            if (value != null && value.startsWith(JIRAConstants.JIRA_ACCOUNT_ID_PREFIX)) {
                value = value.substring(JIRAConstants.JIRA_ACCOUNT_ID_PREFIX.length());
                JSONObject nameObj = new JSONObject();
                nameObj.put("accountId", value);
     ...
     */
    public JIRATimesheetData getSPTimesheetData(final XMLGregorianCalendar dateFrom,final XMLGregorianCalendar dateTo, String projectKey, String author, double spToHoursRatio, Date[] tsDays, boolean[] tsWorkDays)
    {
     ...
     */
    public JIRATimesheetData getWorkLogsTimesheetData(final XMLGregorianCalendar dateFrom, final XMLGregorianCalendar dateTo, String projectKey, String author)
    {
    ...
    {
        List<JIRAIssueWork.JIRAWorklogEntry> validWorklogs = new ArrayList<>();
     ...
     *
     * TODO Implement a more reliable way to retrieve user Account ID from username.
     */
    public String getAccountIdFromLogonUsername(String logonUsername) {
        ...
        try {
            ...
            if (results == null || results.length() == 0) {
                throw new RuntimeException("The search for user " + logonUsername + " didn't return any match when looking for Timesheet user Account ID");
            }

            if (results.length() > 1) {
                throw new RuntimeException("The search for user " + logonUsername + " returned more than one match when looking for Timesheet user Account ID");
            }

            JSONObject userInfo = results.getJSONObject(0);

            return userInfo.getString("accountId");
        } catch (JSONException e) {
            throw new RuntimeException("Error when retrieving User account ID for user " + logonUsername, e);
        }
    }

    public String getJiraAccountIdFromPpmUser(com.ppm.integration.agilesdk.dm.User user) {
        // We search first by email, or by full name, whatever comes first.
        // Jira Cloud doesn't offer the possibility to search by username for GDPR-related reasons.
