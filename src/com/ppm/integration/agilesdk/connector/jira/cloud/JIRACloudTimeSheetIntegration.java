...
import com.ppm.integration.agilesdk.connector.jira.cloud.model.JIRAExternalWorkItem;
...
import com.ppm.integration.agilesdk.connector.jira.cloud.model.JIRATimesheetData;
...
public class JIRACloudTimeSheetIntegration extends TimeSheetIntegration {
    ...
    public List<ExternalWorkItem> getExternalWorkItems(TimeSheetIntegrationContext timesheetContext, ValueSet values) {
        ...
        String authorUsername = values.get(JIRAConstants.KEY_USERNAME);

        String authorAccountId = s.getAccountIdFromLogonUsername(authorUsername);
        ...
        if (JIRAConstants.TS_IMPORT_HOURS_SP_ONLY.equals(importEffortBy)) {
            timesheetData = s.getSPTimesheetData(start, end, projectKey, authorAccountId, spToHoursRatio,
                    tsDays, tsWorkDays);
        } else if (JIRAConstants.TS_IMPORT_HOURS_BOTH.equals(importEffortBy)) {
            timesheetData = s.getWorkLogsTimesheetData(start, end, projectKey, authorAccountId);
            mergeTimeSheetData(timesheetData, s.getSPTimesheetData(start, end, projectKey, authorAccountId, spToHoursRatio, tsDays, tsWorkDays));
        } else {
        ...
        if (JIRAConstants.TS_GROUP_BY_PROJECT.equalsIgnoreCase(groupBy)) {
            ...
            for (Map.Entry<String, Set<String>> issuesInEpic : data.getIssuesInEpics().entrySet()) {
                ...
                if (epicKey == null || "null".equals(epicKey)) {
                    ...
                    timesheetLines.addAll(getTimesheetLinesGroupedByProject(issuesPerProject, data.getEffortPerIssue(), s, Providers.getLocalizationProvider(JIRACloudIntegrationConnector.class).getConnectorText("NO_EPIC_TIMESHEET_LINE_PREFIX")));
