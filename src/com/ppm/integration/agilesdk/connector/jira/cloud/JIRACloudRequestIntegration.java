    ...
    private Map<String,String> getFieldsFromAgileEntity(AgileEntity entity, Map<String, JIRAFieldInfo> fieldsInfo, JIRAService service) {
        ...
        while (fieldsIterator.hasNext()) {
            ...
            if(fieldInfo.getType().equals(JIRAConstants.KEY_FIELD_TYPE_ARRAY) && fieldInfo.getItems().equals(JIRAConstants.KEY_FIELD_TYPE_USER)){
                ...
                if (dataField.isList() && dataField.getType().equals(DATA_TYPE.USER)) {
                    ...
                    if (users != null && !users.isEmpty()) {
                        for(int i=0;i<users.size();i++){
                        	String jiraUserAccountId = service.getJiraAccountIdFromPpmUser(users.get(i));
                        	if(jiraUserAccountId != null){
                        		try {
									JSONObject obj = new JSONObject();
									obj.put("accountId", jiraUserAccountId);
									tempArr.put(obj);
                    ...
                    if (user == null) {
                        ...
                        String jiraUserAccountId = service.getJiraAccountIdFromPpmUser(user);
                        fields.put(field.getKey(), jiraUserAccountId);
                    }
                } else if(dataField.getType().equals(DATA_TYPE.STRING) || dataField.getType().equals(DATA_TYPE.MEMO)){
                	JSONArray ary = new JSONArray();
                	JSONObject obj = new JSONObject();
                	String jiraUserAccountId = "";
                	String str = dataField.get().toString();
                	User tempUser= new User();
                	tempUser.setEmail(str);
                	jiraUserAccountId = service.getJiraAccountIdFromPpmUser(tempUser);
                	if(jiraUserAccountId != null){
                		try {
							obj.put("accountId", jiraUserAccountId);
							ary.put(obj);
                ...
                if (dataField.isList() && dataField.getType().equals(DATA_TYPE.USER)) {
                    ...
                    if (users != null && !users.isEmpty()) {
                    	for(User temUser: users){
                    		String existJiraUsername = service.getJiraAccountIdFromPpmUser(temUser);
                    		if(existJiraUsername != null){
                    			jiraUserAccountId = existJiraUsername;
                        		break;
                    		}
                    	}
                    }
                    ...
                    if (user == null) {
                    	...
                    	jiraUserAccountId = service.getJiraAccountIdFromPpmUser(user);
                    }
                } else if(dataField.getType().equals(DATA_TYPE.STRING) || dataField.getType().equals(DATA_TYPE.MEMO)){
                	String str = dataField.get().toString();
                	User user = new User();
                ...
                if (jiraUserAccountId != null && jiraUserAccountId.isEmpty()) {
                    fields.put(field.getKey(), null);
                } else {                   
                    fields.put(field.getKey(), jiraUserAccountId == null ? null : JIRA_ACCOUNT_ID_PREFIX + jiraUserAccountId);
                }
